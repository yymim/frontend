/*
  ...
  ! 변수 선언
    ...
    값을 저장할 컴퓨터 메모리 공간에 이름을붙이는 것

  ! 변수명 명명규칙
    - 첫 글자 : 영문자, 언더스코어, 달러만 사용 가능 (이후 숫자 사용 가능)
    - 띄어쓰기 허용하지 않음
    - 기호는 _, $만 사용 가능
    - 영어 대소문자 구별
...
document.write('태어난 연도 '+ birthYear +'에 태어난 사람의 나이는 ' + age + '세 입니다.');
*/

/*
  ! 상수(Constant)
    : 한 번 할당된 값을 변경할 수 없음 (재할당 불가)
    : 상수의 선언과 동시에 할당이 반드시 이루어져야 함(초기화)
...
// 변수가 속하는 범위가 애매하다는 점에서 코드에 혼란을 줄 수 있기 때문에 사용하지 않음
